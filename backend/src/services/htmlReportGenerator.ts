...
<body>
  ...
  private renderClaimVerificationSection(
    claimSupport: ClaimSupportV1[] | undefined,
    ...
    const supportStatuses = verification?.claimResults?.length
      ? verification.claimResults.map(claim => claim.status)
      : (claimSupport || []).map(claim => claim.supportLevel);
    ...
    <div class="section">
      ...
      <table class="claim-source-table">
        <thead>
          <tr>
            <th>${this.escapeHtml(localize(outputLanguage, '级别', 'Severity'))}</th>
            <th>${this.escapeHtml(localize(outputLanguage, '断言', 'Claim'))}</th>
            <th>${this.escapeHtml(localize(outputLanguage, '代码', 'Code'))}</th>
            <th>${this.escapeHtml(localize(outputLanguage, '说明', 'Message'))}</th>
          </tr>
        </thead>
        <tbody>${issueRows}</tbody>
      </table>` : ''}
      ${identitySummary ? `
      <div class="claim-source-note">${this.escapeHtml(localize(outputLanguage, '身份解析 Sidecar', 'Identity Resolution Sidecars'))}</div>
      <table class="claim-source-table">
        <thead>
          <tr>
            <th>${this.escapeHtml(localize(outputLanguage, 'ID', 'ID'))}</th>
            <th>${this.escapeHtml(localize(outputLanguage, '状态', 'Status'))}</th>
            <th>${this.escapeHtml(localize(outputLanguage, '进程', 'Process'))}</th>
            <th>${this.escapeHtml(localize(outputLanguage, '线程', 'Thread'))}</th>
          </tr>
        </thead>
        <tbody>${identitySummary}</tbody>
      </table>` : ''}
    </div>`;
  }
