...
> **取代**: v1（`improvements-v1.md`）的"补丁式"思路
...
- **后续 contributor 看哪份**：
  ...
  - 想知道 SDK 限制 → `docs/archive/context-engineering/sdk-capability-spike-2026-04-28.md`
  - 想知道 Phase 3-3 何时该重启 → `docs/archive/context-engineering/v2.1-phase-3-3-deferred.md`
  - 想知道 phase_hints 命运决策 → `docs/archive/context-engineering/v2.1-phase-2.5-decision.md`
  - 想知道 Phase 3 active compact 设计 → `docs/archive/context-engineering/v2.1-phase-3-active-compact-design.md`
...
- **1.0 ✅ 已完成** SDK capability spike：见 `docs/archive/context-engineering/sdk-capability-spike-2026-04-28.md`。**关键发现**：`@anthropic-ai/claude-agent-sdk` 的 `systemPrompt` 类型是 `string | {type:'preset',preset:'claude_code',append?}` —— **不接受 `cache_control` block，无法显式配置 1h TTL**。Query handle 也只暴露 `interrupt()` / `setPermissionMode()`，无 mid-stream message push API
...
- **2.5 ✅ 已决策 C 路径**（见 `docs/archive/context-engineering/v2.1-phase-2.5-decision.md`）：保留 3 个 scene 的 `phase_hints`（scrolling/startup/anr）+ 明确 `plan_template` vs `phase_hints` 的角色分工：前者门控 plan 提交，后者在 mid-execution（`update_plan_phase` / `fetch_artifact(full|rows)`）注入约束。9 个无 phase_hints 的 scene 不强制补，避免 plan_template 与 phase_hints 重复。

### Phase 3: SDK-aware compact + bounded recovery（拆 4 个 PR）

> **SDK spike 结果（2026-04-28，见 `docs/archive/context-engineering/sdk-capability-spike-2026-04-28.md`）**：
...
> （详见 `docs/archive/context-engineering/v2.1-phase-3-active-compact-design.md`）。
...
- **Commit**：`6165304`
- **关键发现**（`docs/archive/context-engineering/sdk-capability-spike-2026-04-28.md`）：
...
- **Commit**：`3ddfdc6` + `docs/archive/context-engineering/v2.1-phase-2.5-decision.md`
...
- **下一步**：见 `docs/archive/context-engineering/v2.1-phase-3-3-deferred.md`，等触发条件再做 full 版本
...
v1 设计文档保留为历史参考：[`improvements-v1.md`](./improvements-v1.md)。
