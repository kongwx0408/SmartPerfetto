...
这个模式与 **[Flutter TextureView 模式](flutter_textureview.md)** 有相似之处，但不能简单视为“一模一样”：两者都依赖 `SurfaceTexture` + 宿主侧再合成，但 SDK 内核的 producer 线程、listener 线程和消费线程可能完全不同。
...
2.  **Invalidate**: 通知 App 的 View System 重绘。
    *   *性能瓶颈*: `onFrameAvailable` 的回调线程取决于 listener / handler 绑定方式；很多实现最终会回到主线程触发 `invalidate()`，但不应写死为“默认主线程”。

### 第三阶段：App RenderThread (Consumer)
1.  **updateTexImage / consume latest image**: 宿主渲染线程在绘制 `TextureView` 节点时，通常会消费 `SurfaceTexture` 中的最新网页帧；具体可见的 slice 名依版本和 trace 配置而异。
...
*   **缺点**: 性能开销通常较大（多一次宿主侧采样/合成、额外回调/同步），内存占用也往往更高。
