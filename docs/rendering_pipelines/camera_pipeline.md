...
*   **MediaCodec Input Surface**: HAL 填充 Buffer -> MediaCodec (Encoder) -> H.264/265 bitstream。
    *   *路径*: 全程硬件加速零拷贝（HAL 直接向 Encoder 的 Input Surface 写入 GraphicBuffer），不经过 CPU。
...
为了解决”按下快门到真正拍照”的延迟：
*   Camera HAL 一直在后台以全分辨率捕获 **reprocessable YUV/RAW** 帧，存入一个环形缓冲区 (Ring Buffer)。
*   当用户按快门时，系统从缓冲区中”捞”出最近的一帧，通过 **Reprocessing Pipeline**（ISP 再处理：降噪、HDR 合成等）生成最终的 JPEG/HEIC 输出。
*   *注*: ZSL 缓冲区存储的是未压缩的 YUV 或 RAW 数据（而非 JPEG），以保留最大后处理空间。Reprocessing 通过 `CameraCaptureSession.createReprocessableCaptureSession()` 配置。
...
## 5. 常见 Trace 分析
在 Perfetto 中：
*   **CameraProvider / CameraService / vendor camera threads**: 若数据源启用且设备支持，可用于观察 HAL / service / request 流转。
...
1.  **Buffer Starvation**: Consumer (`ImageReader`) 处理太慢，`image.close()` 不及时，导致 HAL 无法获取空闲 Buffer。
    *   *Trace*: 可能看到 Camera / BufferQueue / binder / consumer 相关等待，但不保证稳定出现 `CameraProvider::dequeueBuffer` 这类 slice。
