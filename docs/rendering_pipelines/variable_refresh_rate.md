...
> [!NOTE]
> **ARR (Adaptive Refresh Rate)**: Android 15 QPR1 引入了 ARR 基础支持，Android 16 增加了 `hasArrSupport()` 和 `getSuggestedFrameRate()` 等便利 API。

Android 11 引入了 `setFrameRate()` 帧率声明 API，Android 12+ 逐步完善 VRR / ARR 管线支持。**可变刷新率 (VRR)** 允许屏幕刷新率在设备支持的模式集合内动态变化，对渲染管线和性能分析带来根本性影响。实际支持范围、切换延迟和最低刷新率取决于面板、OEM 策略和内容场景。
...
| 特性 | 固定刷新率 | VRR |
|:---|:---|:---|
| **VSync 周期** | 固定 (如 16.6ms @ 60Hz) | **动态**（取决于设备支持的刷新模式） |
...
// 声明场景类型，系统自动优化
// Android 15 QPR1+ View-level Frame Rate Category
...
// 投票机制：多个 View 各自声明帧率需求，系统综合决策
// View hierarchy 中每个 View 的 setRequestedFrameRate() 会被自动聚合
```
...
-- 正确的掉帧检测 (VRR 感知, Perfetto stdlib)
INCLUDE PERFETTO MODULE android.frames;
SELECT frame_id, ts, dur, jank_type
FROM android_frames
WHERE jank_type != 'None';  -- ✅ 使用 stdlib 的 jank 检测
```
...
| Android 版本 | VRR 支持 |
|:---|:---|
| **Android 16** (API 36) | ARR / frame-rate API 继续演进（仍依硬件支持） |
| **Android 15** (API 35) | 改进 ARR / view-level frame-rate 能力 |
| Android 14 (API 34) | VRR 策略继续完善（需硬件支持） |
...
1.  **明确帧率意图**: 不要依赖系统猜测，主动调用 `setFrameRate()` 或对应版本可用的 view-level frame-rate API
2.  **静态检测**: 检测无动画时主动降频
